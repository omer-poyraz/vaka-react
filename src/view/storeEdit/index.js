...
import { CreateStoreWorkOrderData, CreateWorkOrderData, DeleteWorkOrderData, StoreWorkOrdersData, UpdateWorkOrderData } from '../../service'
...
const StoreEdit = () => {
    ...
    const getData = () => {
        dispatch(StoreWorkOrdersData({ sId: selector.route.storeId }))
    }
            ...
            <Card>
                <CardHeader>
                    <div className='d-flex justify-content-between align-items-center'>
                        <div><h3>Store Work Order</h3></div>
                ...
                <CardBody>
                    <Table
                        bordered
                        columns={columns}
                        dataSource={selector.storeWorkOrders.storeWorkOrders}
                    />
                </CardBody>
            </Card>
            <Modal isOpen={modal} toggle={() => setModal(!modal)} className='modal-dialog-centered'>
                <ModalHeader toggle={() => setModal(!modal)} tag='h3'>{isCreate ? "Add Store Work Order" : "Edit Store Work Order"}</ModalHeader>
                <ModalBody>
