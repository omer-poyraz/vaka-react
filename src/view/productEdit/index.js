...
import { CreateProductWorkOrderData, CreateWorkOrderData, DeleteWorkOrderData, ProductWorkOrdersData, UpdateWorkOrderData } from '../../service'
...
const ProductEdit = () => {
    ...
    const getData = () => {
        dispatch(ProductWorkOrdersData({ pId: selector.route.productId }))
    }
    ...
    const createSubmit = (values, e) => {
        console.log(selector.route.productId)
        dispatch(CreateProductWorkOrderData({
            pId: selector.route.productId,
            ...
            <Card>
                <CardHeader>
                    <div className='d-flex justify-content-between align-items-center'>
                        <div><h3>Product Work Order</h3></div>
                ...
                <CardBody>
                    <Table
                        bordered
                        columns={columns}
                        dataSource={selector.productWorkOrders.productWorkOrders}
                    />
                </CardBody>
            </Card>

            <Modal isOpen={modal} toggle={() => setModal(!modal)} className='modal-dialog-centered'>
                <ModalHeader toggle={() => setModal(!modal)} tag='h3'>{isCreate ? "Add Product Work Order" : "Edit Product Work Order"}</ModalHeader>
                <ModalBody>
